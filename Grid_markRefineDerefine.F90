...
!!  hlaCluster
!! NO: Modified, now excepts grv_nsinks, and calls modified gr_unmarkRefineByLogRadius
!!
!! NAME
...
!!$  use physicaldata, ONLY : force_consistency
  ...
  use Gravity_data, ONLY: grv_ptxpos, grv_ptypos, grv_ptzpos, grv_ptmass, grv_totmass
...
!!$  force_consistency = .TRUE.
  ...
  !! maximally refine around each binary component, scaled to bondi radius of component
  !! To do: add functionality for two-level maximum refinement
  call gr_markInRadius(grv_ptxpos(1), grv_ptypos(1), grv_ptzpos(1),&
                       sim_radius*sim_max_ref_rad*grv_ptmass(1)/grv_totmass, gr_maxRefine)
  call gr_markInRadius(grv_ptxpos(2), grv_ptypos(2), grv_ptzpos(2),& 
                       sim_radius*sim_max_ref_rad*grv_ptmass(2)/grv_totmass, gr_maxRefine)
  ...
  ! force derefinement away from binary
  if(gr_lrefineMaxRedDoByLogR) & 
       call gr_unmarkRefineByLogRadius(grv_ptxpos,grv_ptypos,grv_ptzpos)
