...
!! hlaBoundary
!! Hoyle-Lyttleton (HL) Simulation
!! The user_defined boundary sets the boundary cells and the adjoining guard cells to
!! the ambient density and pressure.  The x-velocity is set to windvel.
...
!! have to negate the values.
  ...
  if(face==LOW) then
    ...
    case(USER_DEFINED)
        ...
        case(VELX_VAR)
           dataRow(1:guard)=0.0 ! NO: Remove sim_windVel
        case(VELY_VAR)
           dataRow(1:guard)=0.0
        case(VELZ_VAR)
           dataRow(1:guard)=0.0
        case(ENER_VAR)
           kine = 0.0 ! 0.5 * (sim_windVel**2) ! NO: zero kinetic energy)
     ...
     case(USER_DEFINED)
        ...
        case(PRES_VAR)
           dataRow(1:guard)=sim_pAmbient
        case(VELX_VAR)
           dataRow(1:guard)=0.0 ! sim_windVel ! NO: zero velocity
        case(VELY_VAR)
           dataRow(1:guard)=0.0
        case(VELZ_VAR)
           dataRow(1:guard)=0.0
        case(ENER_VAR)
           kine = 0. ! 0.5 * (sim_windVel**2) ! NO: zero kinetic energy
           dataRow(1:guard) = max( sim_pAmbient / ((sim_gamma-1.) * sim_rhoAmbient) + kine, sim_smallP)
        case(EINT_VAR)
           dataRow(1:guard) = max( sim_pAmbient / ((sim_gamma-1.) * sim_rhoAmbient), sim_smallP)
        case(SPECIES_BEGIN)
